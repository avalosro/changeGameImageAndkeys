	...
	_vertexBufferID(0)
{
	...
	_textureBufferID = loadAndBufferImage("pene.png");
	_ballTextureBufferID = loadAndBufferImage("espermas.png");
...
void GameWindow::keyButtonPressed(int button, int action)
{
	...
	if (button == GLFW_KEY_J && action == GLFW_PRESS) 
	{
		...
		ball = new Sprite(_window, _ballTextureBufferID, makeVector2(_playerRocket->getPosition().x+SQUARE_SIZE,_playerRocket->getPosition().y));
		ball->setVelocity(makeVector2(8.0, 0.0));
